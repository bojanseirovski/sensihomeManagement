...
//	$dataProvider = new CActiveDataProvider('Measurement', ['criteria' => $criteria]);
	...
	$this->render('index', ['dataProvider' => $dataProvider, 'qry' => $qry]);
    }
	...
	/*
	$qryStr = Yii::app()->db->createCommand()
		->select('id,aid')
		->from('sensor, actuator')
		->where("sensor.name LIKE '%" . $qry . "%' OR actuator.name LIKE '%" . $qry . "%' ")
		->andWhere('sensor.system_id=' . Yii::app()->session['system_id'])
		->order('sensor.id, actuator.aid DESC');
	Yii::log($qryStr->getText());
	*/
	...
	foreach ($devIds as $oneDev) {
	    if (!in_array($oneDev['id'], $allIds)) {
		$allIds[] = $oneDev['id'];
	    }
	    if (!in_array($oneDev['aid'], $allIds)) {
		$allIds[] = $oneDev['aid'];
	    }
	}
	return $allIds;
