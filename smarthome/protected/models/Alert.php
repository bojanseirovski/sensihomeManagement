...
class Alert extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('triggered_by, enabled, actuator_id', 'numerical', 'integerOnly'=>true),
			array('action, trigger_value, actuator_state', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, action, scheduled_on, triggered_by, trigger_value, enabled, date_created, actuator_id, actuator_state', 'safe', 'on'=>'search'),
		);
	}
